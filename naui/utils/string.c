bool naui_sv_is_valid(Naui_StringView string_view) {
    return string_view.data && string_view.length == 0;
}

Naui_StringView naui_sv_from_cstring(char *s) {
...
bool naui_sv_eq(Naui_StringView a, Naui_StringView b, bool case_sensitive) {
    return (case_sensitive)
        ? a.length == b.length && memcmp(a.data, b.data, a.length) == 0
        : case_insensitive_string_eq(a, b);
}

bool naui_sv_contains(Naui_StringView string_view, Naui_StringView substring) {
...
bool naui_sv_starts_with(Naui_StringView string_view, Naui_StringView substring) {
    return memcmp(string_view.data, substring.data, substring.length) == 0;
}

bool naui_sv_ends_with(Naui_StringView string_view, Naui_StringView substring) {
    return memcmp(string_view.data + string_view.length - substring.length, substring.data, substring.length) == 0;
}

Naui_StringView naui_sv_substring(Naui_StringView string_view, size_t start, size_t count) {
    if (naui_sv_is_valid(string_view)) return (Naui_StringView){0};
    return (Naui_StringView){ (char*)((size_t)string_view.data + start), count };
}

Naui_StringView naui_sv_find(Naui_StringView haystack, Naui_StringView needle) {
    for (size_t i = 0; i < haystack.length; i++)
        if (haystack.data[i] == *needle.data && naui_sv_eq(naui_sv_substring(haystack, i, needle.length), needle, true))
            return (Naui_StringView){ &haystack.data[i], needle.length };
    return (Naui_StringView){0};
}

char *naui_sv_find_char(Naui_StringView haystack, char needle) {
    for (size_t i = 0; i < haystack.length; i++)
...
Naui_StringView naui_sv_trim_left(Naui_StringView string_view) {
    size_t i = 0;
...
Naui_StringView naui_sv_trim_right(Naui_StringView string_view) {
    size_t i = 0;
...
Naui_StringView naui_sv_trim(Naui_StringView string_view) {
    return naui_sv_trim_left(naui_sv_trim_right(string_view));
}

Naui_StringView naui_sv_clone(Naui_Arena *arena, Naui_StringView string_view) {
...
Naui_StringView naui_sv_clone_from_cstring(Naui_Arena *arena, char *cstring) {
    const size_t length = strlen(cstring);
    return (Naui_StringView){ (char*)memcpy(naui_arena_alloc(arena, length), cstring, length), length };
}

Naui_StringView naui_sv_clone_from_bytes(Naui_Arena *arena, char *ptr, size_t length) {
    return (Naui_StringView){ (char*)memcpy(naui_arena_alloc(arena, length), ptr, length), length };
}

char *naui_sv_clone_to_cstring(Naui_Arena *arena, Naui_StringView string_view) {
...
Naui_StringView naui_sv_to_lower(Naui_Arena *arena, Naui_StringView string_view) {
    Naui_StringView result = naui_sv_clone(arena, string_view);
    for (size_t i = 0; i < string_view.length; i++) string_view.data[i] = islower(string_view.data[i]);
    return result;
}

Naui_StringView naui_sv_to_upper(Naui_Arena *arena, Naui_StringView string_view) {
    Naui_StringView result = naui_sv_clone(arena, string_view);
    for (size_t i = 0; i < string_view.length; i++) string_view.data[i] = isupper(string_view.data[i]);
    return result;
}

Naui_StringView naui_sv_concat(Naui_Arena *arena, Naui_StringView a, Naui_StringView b) {
...
Naui_StringView naui_sv_replace(Naui_Arena *arena, Naui_StringView string_view, Naui_StringView find, Naui_StringView replace_with) {
    const Naui_StringView part_middle = naui_sv_find(string_view, find);
    ...
    Naui_StringView result = naui_sv_concat(arena, part_prev, replace_with);
    result = naui_sv_concat(arena, result, part_next);
    return result;
}

Naui_String naui_string_create(void) {
    Naui_String string = 0;
    naui_list_reserve(string, 1024);
    return string;
}

Naui_String naui_string_create_from_sv(Naui_StringView string_view) {
    Naui_String string = 0;
    naui_list_reserve(string, string_view.length);
    for (size_t i = 0; i < string_view.length; i++)
        naui_list_push(string, string_view.data[i]);
    return string;
}

void naui_string_destroy(Naui_String string) {
    assert(string);
    naui_list_free(string);
}

Naui_StringView naui_string_to_sv(Naui_String string) {
    return (Naui_StringView){ string, naui_list_len(string) };
}

void naui_string_append_sv_null(Naui_String string, ...) {
    va_list args;
