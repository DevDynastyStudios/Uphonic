...
{
    Free,       // Modal floats freely — no input blocking, overlay is purely visual
    SoftBlock,  // Overlay dims the background but other windows remain interactive
    HardBlock,  // Input is locked to the modal stack; nothing behind can be clicked
};

enum class ModalPosition
{
    Default,        // ImGui decides (top-left, as usual)
    Center,         // Centered on the main viewport on first appearance
    TopCenter,      // Horizontally centered, near the top
    Custom,         // Use SetCustomPosition()
};
...
NAUI_API void   SetModalOverlayAlpha(float alpha);  // Convenience — preserves RGB, changes only alpha
