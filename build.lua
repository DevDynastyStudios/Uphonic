...
local PANELS_DIR = "app/panels"

local function list_panel_sources()
    local files = {}

    local cmd
    if IS_WINDOWS then
        cmd = ('dir /s /b "%s\\*.c" 2^>nul'):format(PANELS_DIR)
    else
        cmd = ('find "%s" -name "*.c" 2>/dev/null'):format(PANELS_DIR)
    end

    local handle = io.popen(cmd)
    if handle then
        for line in handle:lines() do
            local rel = line:gsub("\\", "/"):match(PANELS_DIR .. "/(.+)$")
            -- skip the generated build.c itself so it never includes itself
            if rel and rel ~= "build.c" then
...
local function generate_panels_build()
    mkdir(PANELS_DIR)

    local panels = list_panel_sources()
    local path = PANELS_DIR .. "/build.c"
    ...
    f:write("// Auto-generated by build.lua on every build. Do not edit, do not commit.\n")
    f:write("// Includes every .c file found under " .. PANELS_DIR .. "/.\n\n")
...
local function compile(release)
    ...
    if not generate_panels_build() then
        return 1
    end
