...
static void uph_song_timeline_render_top_ruler(Leaf_BoundingBox bbox)
{
    ...
    for (uint32_t i = 0; i < line_count; i++)
    {
        ...
        naui_draw_line(
            naui_vec2(x, bbox.y + bbox.height * 0.5f),
            naui_vec2(x, bbox.y + bbox.height),
            LEAF_COLOR_WHITE,
            1.0f
        );

        naui_draw_text(naui_vec2(x + 4.0f, bbox.y + bbox.height * 0.5f), "0", NAUI_DPI(13.0f), 0, LEAF_COLOR_WHITE);
    }
}
...
static void uph_song_timeline_render_track_header(const Uph_Track *track)
{
    leaf({
        ...
        .size = {LEAF_SIZE_FIXED(NAUI_DPI(140.0f)), LEAF_SIZE_FULL},
    ...
    {
        leaf({
            .size = {LEAF_SIZE_FIXED(NAUI_DPI(4.0f)), LEAF_SIZE_FULL},
            .color = track->color,
...
static float uph_song_timeline_max_scroll_y(float viewport_height)
{
    const uint32_t track_count = (uint32_t)naui_list_len(uph_state.project.tracks);
    const float content_height = (float)track_count * NAUI_DPI(uph_song_timeline_data.zoom.y);
    return fmaxf(0.0f, content_height - viewport_height);
}

static void uph_song_timeline_update_input(float viewport_height)
{
    const float wheel_y = (float)naui_mouse_scroll_delta();
    const bool ctrl_held = naui_key_down(NAUI_KEY_LCONTROL);
    const float max_scroll_y = uph_song_timeline_max_scroll_y(viewport_height);
...
static void uph_song_timeline_on_update(void)
{
    ...
    uph_song_timeline_update_input(track_section_bbox.height);
