...
{
    ProjectState& state = ProjectState::GetInstance();
    float playheadX = canvasPos.x + m_config.trackHeaderWidth + 
                      state.timelinePositionBeats * beatWidth - m_scrollX;
    draw->AddLine(ImVec2(playheadX, canvasPos.y),
                           ...
                           TrackUIState& ui)
{
    ...
    float instX = trackStartX + instance.midiBlock.startBeat * beatWidth - m_scrollX;
    float instWidth = instance.midiBlock.lengthBeats * beatWidth;
    float instY = yPos + m_config.instancePadding;
    float instHeight = ui.height - m_config.instancePadding * 2;
    float instStartOffset = instance.midiBlock.startOffsetBeats * beatWidth;

    if (instX + instWidth < trackStartX || instX > canvasPos.x + canvasSize.x) return;
    
    bool selected = IsInstanceSelected(trackIndex, instanceIndex);
        ...
        instance.midiBlock.patternIndex < (int)state.patterns.size())
    {
        ...
        if (!pattern.notes.empty())
        {
            ...
            if (noteAreaHeight > 10.0f)
            {
                ...
                for (const auto& note : pattern.notes)
                {
                    float noteX = instX - instStartOffset + (note.startBeat / instance.midiBlock.lengthBeats) * instWidth;
                    float noteW = (note.lengthBeats / instance.midiBlock.lengthBeats) * instWidth;
                    ...
                    draw->AddRectFilled(ImVec2(noteX, noteY),
                                    ImVec2(noteX + noteW, noteY + noteHeight),
                                    noteCol, 1.0f);
                }
            }
        }
                         ...
                         TrackUIState& ui)
{
    ...
    float instX = trackStartX + instance.sampleBlock.startBeat * beatWidth - m_scrollX;
    float instWidth = instance.sampleBlock.lengthBeats * beatWidth;
    float instY = yPos + m_config.instancePadding;
    float instHeight = ui.height - m_config.instancePadding * 2;
...
void SongTimeline::HandleInput(ImVec2 canvasPos, ImVec2 canvasSize, float beatWidth)
{
    ...
    if (!ImGui::IsWindowHovered() && !isInteracting)
    {
        return;
    }
                     ...
                     float trackStartX, float beatWidth)
{
    ...
    if (instTrack >= 0 && instIdx >= 0)
    {
        ...
        float instStartX = trackStartX + blockStart * beatWidth - m_scrollX;
        float instEndX = instStartX + blockLength * beatWidth;
                            ...
                            ImVec2 mousePos, float trackStartX, float beatWidth)
{
    ...
    if (instIdx == -1 && trackIdx >= 0)
    {
        ...
        int newIdx = track.blocks.size() - 1;
...
void SongTimeline::HandleSingleInstanceDrag(int hoverTrack, double deltaBeat, float timelineStartY)
{
    ...
    if (hoverTrack >= 0 && hoverTrack != m_selection.draggedTrack)
    {
        ...
        if (srcTrack.type == dstTrack.type && m_selection.draggedInstance < (int)srcTrack.blocks.size())
        {
            ...
            m_selection.draggedInstance = dstTrack.blocks.size() - 1;
                           ...
                           float trackStartX, float beatWidth)
{
    ...
    for (size_t t = 0; t < state.tracks.size(); t++)
    {
        ...
        for (size_t i = 0; i < track.blocks.size(); i++)
        {
            ...
            float instX1 = trackStartX + blockStart * beatWidth - m_scrollX;
            float instX2 = instX1 + blockLength * beatWidth;
            ...
            if (inBox && !IsInstanceSelected(t, i))
            {
