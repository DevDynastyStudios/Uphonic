...
void FileExplorer::RenderToolbar()
{
	...
	if (ImGui::InputTextWithHint("##Path", "Path...", m_pathInput, sizeof(m_pathInput), ImGuiInputTextFlags_EnterReturnsTrue))
	{
		...
		ImGuiTableFlags_BordersInnerV))
	{
		...
		while (clipper.Step())
		{
			for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++)
			{
				...
				if (ImGui::Selectable(("##row" + std::to_string(i)).c_str(), selected, ImGuiSelectableFlags_SpanAllColumns | ImGuiSelectableFlags_AllowDoubleClick))
				{
					m_selectedIndex = i;
